#!/usr/bin/perl -w
#
# $FreeBSD: CVSROOT/log_accum.pl,v 1.120 2003/02/06 23:49:42 peter Exp $
#
...
sub mail_notification {
	...
	{
		$^W=0;
		$subject = sprintf("Subject: Warning: user %s checkin frozen branch: %s!!!",
				$uid, join(", ", @frozentags));
		$^W=1;
	}
...
my ($tpattern, $userlist, $match, $branch_open) = ("","",0,0);

use vars qw(@frozentags $uid);
$uid = $cfg::COMMITTER;
@frozentags = ();
...
while (<STDIN>) {
	...
	for my $i (0 .. ($#cfg::TAG_ACCESS_MAP - 1) / 2) {
		$userlist = $cfg::TAG_ACCESS_MAP[$i * 2];
		$tpattern = $cfg::TAG_ACCESS_MAP[$i * 2 + 1];
		
		if ($tag =~ /$tpattern/i)
		{
			$match = 1 unless ($tpattern eq ".*");
	...
	$branch_open = $branch_open || !$match;
	if (!$branch_open)
	{
		if (@frozentags)
		{
			push @frozentags, $tag if ($tag ne $frozentags[-1]);
		}
		else
		{
			push @frozentags, $tag;
...
$warning_msg = sprintf("Error: branch \"%s\" is frozen, you should not commit to this branch!\n", join(", ", @frozentags)) if (@frozentags);
...
for (my $i = 0; ; $i++) {
	...
	push @log_msg, $warning_msg if ($warning_msg);
